...
import {
  ...
  articleAuthorRow, authorInfo, articleContent, articleFooter, articleActions,
  editBtn, deleteBtn, loadingClass, errorClass, inputClass, commentsWrapper,
  commentCard, commentHeader, commentUserRow, avatar, commentUser, commentTime, commentText,
...
function ArticleByID() {
  ...
  const formatDate = (date) => new Date(date).toLocaleString("en-IN", { timeZone: "Asia/Kolkata", dateStyle: "medium", timeStyle: "short" });

  const toggleArticleStatus = async () => {
    const newStatus = !article.isArticleActive;
    if (!window.confirm(newStatus ? "Restore this article?" : "Delete this article?")) return;
    try {
      const res = await axios.patch(`${API_BASE_URL}/author-api/articles`, { articleId: article._id, isArticleActive: newStatus }, { withCredentials: true });
      setArticle(res.data.payload);
    } catch (err) {
  ...
  const addComment = async (commentObj) => {
    try {
      const res = await axios.put(`${API_BASE_URL}/user-api/articles/${article._id}/comments`, { comment: commentObj.comment }, { withCredentials: true });
      if (res.status === 200) setArticle(res.data.payload);
    } catch (err) { console.error("Comment failed:", err.response?.data || err.message); }
  };
